...
function readLegacyOllamaModel(
  value: unknown,
...
function readLegacyOpenAICompatibleModel(value: unknown): {
  baseUrl: string
  apiKey: string
  model: string
} {
...
export const migrateFrom1To2: SettingMigration['migrate'] = (
  ...
  const ollamaChatModel = readLegacyOllamaModel(data.ollamaChatModel)
  const ollamaApplyModel = readLegacyOllamaModel(data.ollamaApplyModel)
  const ollamaEmbeddingModel = readLegacyOllamaModel(data.ollamaEmbeddingModel)
  const openAICompatibleChatModel = readLegacyOpenAICompatibleModel(
    data.openAICompatibleChatModel,
  )
  const openAICompatibleApplyModel = readLegacyOpenAICompatibleModel(
  ...
  const defaultOllamaBaseUrl =
    ollamaEmbeddingModel.baseUrl || // embedding model takes precedence
    ollamaChatModel.baseUrl ||
    ollamaApplyModel.baseUrl ||
    ''
