...
const OPENAI_EMBEDDING_MODEL_ID = 'openai/text-embedding-3-small'
const OLLAMA_EMBEDDING_MODEL_ID = 'ollama/nomic-embed-text'

function withLegacyEmbeddingSettings(
  embeddingModelId: string,
  ollamaEmbeddingModel: { baseUrl: string; model: string } = {
    baseUrl: '',
    model: '',
  },
) {
...
function withMigratedEmbeddingSettings(embeddingModelId: string) {
  return {
...
describe('settings 1_to_2 migration', () => {
  it('should migrate from v1 to v2', () => {
    const oldSettings = {
      ...
      ...withLegacyEmbeddingSettings(OLLAMA_EMBEDDING_MODEL_ID, {
        baseUrl: 'http://127.0.0.1:11434',
    ...
    expect(result).toEqual({
      ...
      ...withMigratedEmbeddingSettings(OLLAMA_EMBEDDING_MODEL_ID),
  ...
  it('should handle minimal v1 settings', () => {
    const oldSettings = {
      ...
      ...withLegacyEmbeddingSettings(OPENAI_EMBEDDING_MODEL_ID),
    ...
    expect(result).toEqual({
      ...
      ...withMigratedEmbeddingSettings(OPENAI_EMBEDDING_MODEL_ID),
  ...
  it('should tolerate missing nested model configs', () => {
    expect(
      migrateFrom1To2({
        version: 1,
        openAIApiKey: '',
