...
export default class SmartComposerPlugin extends Plugin {
  ...
  async onload() {
    ...
    if (shouldAutoStartLocalEmbeddingService(this.settings)) {
      void ensureLocalEmbeddingService(this.manifest.dir).then((started) => {
        if (!started) {
          new Notice('本地嵌入服务未能自动启动，请检查本地 BGE 服务。', 5000)
        }
      })
    }
    ...
    this.addRibbonIcon('wand-sparkles', 'Open AutoFlow Agent', () =>
      this.openChatView(),
    )
